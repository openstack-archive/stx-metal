 ...
 */
  ...
  *
  * This file implements the a set of secre utilities that maintenance
                                                 ...
                                                 void (*handler)(int, siginfo_t*, void*))
{
         ...
         it->second.stage == MTC_SECRET__GET_REF_FAIL )
    {
        ...
        if (rc)
        {
            ...
            it->second.stage = MTC_SECRET__GET_REF_FAIL;
            mtcTimer_start( secret_timer, handler, SECRET_RETR_DELAY );
        }
        else
        {
            mtcTimer_start( secret_timer, handler, SECRET_RECV_DELAY );
        }
    }
              ...
              it->second.stage == MTC_SECRET__GET_PWD )
    {
        ...
        else if ( event.base )
        {
            ...
            {
                elog ( "%s getting secret reference timeout \n", host_uuid.c_str() );
                it->second.stage = MTC_SECRET__GET_REF_FAIL ;
                mtcTimer_start( secret_timer, handler, SECRET_RETR_DELAY );
            }
            if ( it->second.stage == MTC_SECRET__GET_PWD )
            {
                elog ( "%s getting secret payload timeout \n", host_uuid.c_str() );
                it->second.stage = MTC_SECRET__GET_PWD_FAIL ;
                mtcTimer_start( secret_timer, handler, SECRET_RETR_DELAY );
            }
 ...
 *              to manage secret's referenceerence.
 *
 */

int secretUtil_get_secret ( libEvent & event, string & host_uuid )
{
    int rc = httpUtil_event_init ( &event,
                                   host_uuid,
...
int secretUtil_handler ( libEvent & event )
{
    ...
    else if ( event.request == BARBICAN_READ_SECRET )
    {
        ...
        {
            ilog ("%s no barbican secret found \n", hn.c_str() );
            event.active = false ;
            it->second.stage = MTC_SECRET__GET_PWD_RECV;
        }
    ...
    {
        elog ("%s Barbican Request Failed - Unsupported Request (%d)\n", hn.c_str(), event.request );
        event.active = false ;
        return (rc);
    }
}
