                         ...
                         const char * value)
{
    ...
    if (MATCH("agent", "mnfa_mode"))
    {
        char * mnfa_mode = strdup(value);

        /* validate and auto correct multi node failure avoidance mode */
        if (( mnfa_mode != NULL ) &&
            ( !strncmp (mnfa_mode, "percent", strlen("percent"))))
        {
            mtcInv.mnfa_mode = MNFA_PERCENT ;
            ilog ("MNFA Mode   : Percent");
        }
        else
        {
            mtcInv.mnfa_mode = MNFA_NUMBER ;
            ilog ("MNFA Mode   : Number");
        }
    ...
    if (MATCH("timeouts", "mnfa_timeout"))
    {
