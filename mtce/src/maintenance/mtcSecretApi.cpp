...
int nodeLinkClass::mtcSecretApi_get_secret ( struct nodeLinkClass::node * node_ptr )
{
    ...
    node_ptr->secretEvent.information = node_ptr->uuid;
...
void nodeLinkClass::mtcSecretApi_get_handler ( struct evhttp_request *req, void *arg )
{
    ...
    if ( node_ptr == NULL )
    {
        slog ("Node Lookup Failed - Barbican Event (%p)\n", arg);
    ...
    if ( ! req )
    {
        elog ("Barbican 'Get' Request Timeout (%s)\n",
    ...
    {
        elog ("Barbican HTTP Get Request Failed (%s) (%d)\n",
                  node_ptr->secretEvent.entity_path.c_str(),
    ...
    {
        elog ("Barbican Server may be down (%s)\n",
               node_ptr->secretEvent.entity_path.c_str());
        goto _get_handler_done ;
    }
    ...
    if ( rc != PASS )
    {
       elog ("Unable to parse Barbican response (%s) (%s)\n",
             node_ptr->secretEvent.entity_path.c_str(),
...
void nodeLinkClass::mtcSecretApi_read_handler ( struct evhttp_request *req, void *arg )
{
    ...
    {
        dlog ("%s Barbican Read (Not-Found) (%d)\n",
    ...
    {
        elog ("%s Barbican HTTP Request Failed (%d)\n",
                  node_ptr->hostname.c_str(),
                  node_ptr->secretEvent.status);
