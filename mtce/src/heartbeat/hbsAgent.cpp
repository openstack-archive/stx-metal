...
void daemon_service_run ( void )
{
    ...
    for ( hbsTimer.ring = false , hbsTimer_audit.ring = false ; ; )
    {
        ...
        if ( hbs_ctrl.audit++ == AUDIT_RATE )
        {
            ...
            if ( daemon_is_file_present ( NODE_LOCKED_FILE ))
            {
                if ( hbsInv.hbs_disabled == false )
                {
                     ...
                     * the throttled count befoer the first log */
                    counter = 1 ;
                }
            }
...
#ifdef WANT_STATE_CHANGE_ON_LINK_EVENTS /* TEST ME: Undo */
        ...
        {
            wlog_throttled (counter, 100000, "Heartbeat service still disabled\n");
            hbsInv.hbs_state_change = false ;
            usleep (50000) ;
            continue ;
        }
