...
static unsigned int    this_controller = 0 ;
...
int _service_pulse_request ( iface_enum iface , unsigned int flags )
{
    ...
    {
        char str[MAX_LEN] ;
        snprintf ( &str[0], MAX_LEN, " seq %6d with %d bytes from %s ", hbs_sock.rx_mesg[iface].s, rx_bytes, get_iface_name_str(iface));
        ...
        else if ( hbs_sock.rx_mesg[iface].cluster.histories )
        {
            ...
            clog2 ("controller-%d cluster info from %s pulse request saved to cache",
            ...
            if ( have_other_controller_history ( controller ) == true )
            {
                ...
                if ( daemon_get_cfg_ptr()->debug_state & 2 )
                {
                    string dump_banner = "" ;
...
void daemon_service_run ( void )
{
    ...
    if ( my_nodetype & CONTROLLER_TYPE )
    {
        ...
        {
            // ilog ("this_controller = 0")
            this_controller = 0 ;
        }
        else
        {
            // ilog ("this_controller = 1")
            this_controller = 1 ;
        }
    }
