...
/****************************************************************************
 *
 * Name        : hbs_cluster_storage0_state
 *
...
void hbs_cluster_storage0_state ( bool enabled )
{
...
void hbs_cluster_add ( string & hostname )
{
    ...
    {
        hbs_cluster_storage0_state ( true );
    }
...
void hbs_cluster_del ( string & hostname )
{
    ...
    if ( hostname == STORAGE_0 )
    {
        hbs_cluster_storage0_state ( false );
    }
                                ...
                                bool storage_0_responding )
{
     ...
     * Typical path for normal non-storage system case */
    else if ( ctrl.storage_0_not_responding_count[n] )
        ctrl.storage_0_not_responding_count[n] = 0 ;
...
void hbs_cluster_send ( msgClassSock * sm_client_sock, int reqid )
{
     ...
     */
    int c = ctrl.controller_0_enabled ;
    c += ctrl.controller_1_enabled ;
#ifdef THREE_CONTROLLER_SYSTEM
    c += ctrl.controller_2_enabled ;
#endif
    int c_copy = c ;
    int n, networks = ctrl.cluster.histories ;
    clog1 ("controllers:%d histories:%d bytes:%d", c, ctrl.cluster.histories, ctrl.cluster.bytes );
    if ( c > 1 )
    {
        for ( c-- ; c > 0 ; c-- )
        {
            for ( n = 0 ; n < networks ; n++ )
            {
                ...
                ctrl.cluster.history[ctrl.cluster.histories].controller = c ;
                ctrl.cluster.bytes += sizeof(mtce_hbs_cluster_history_type) ;
                ctrl.cluster.histories++ ;
                clog1 ("controller-%d network:%d histories:%d bytes:%d", c, networks, ctrl.cluster.histories, ctrl.cluster.bytes );
            }
        }
    }
