...
void hbs_cluster_send ( msgClassSock * sm_client_sock, int reqid )
{
    ...
    if (( sm_client_sock ) && ( sm_client_sock->sock_ok() == true ))
    {
        ...
        else
        {
            ...
            hbs_cluster_dump ( ctrl.cluster, "SM cluster query", true );
        }
    }
...
void hbs_history_save ( mtce_hbs_cluster_history_type & sample )
{
    ...
    for ( int h = 0 ; h < ctrl.cluster.histories ; h++ )
    {
            ...
            ( ctrl.cluster.history[h].network == sample.network ))
        {
            ...
            clog ("controller-%d updated vault with controller-%d:%s network history (histories:%d)",
                   ctrl.this_controller,
                   sample.controller,
...
{
    /* cluster info is only supported in HBS_MESSAGE_VERSION_1 */
    if ( msg.v < HBS_MESSAGE_VERSION_1 )
    ...
    for ( int h = 0 ; h < msg.cluster.histories ; h++ )
    {
        ...
        {
            hbs_history_save ( msg.cluster.history[h] );
        }
