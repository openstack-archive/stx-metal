    ...
    def __get_backend(self):
        if not self.__backend:
            ...
            if backend_name not in self.__backends:
                ...
                raise exception.InventoryException(msg)
...
def notify_mtc_and_recv(mtc_address, mtc_port, idict):
    ...
    try:
        ...
        LOG.exception(_("Socket Error: %(e)s mtc %(mtc_address)s:%(mtc_port)s "
                        "%(serialized_idict)s") %
                      ({'e': e,
                        'mtc_address': mtc_address,
                        'mtc_port': mtc_port,
                        'serialized_idict': serialized_idict}))
        pass
...
def _get_cinder_device(dbapi, host_id):
    if not host_id:
    ...
    i_stors = dbapi.istor_get_by_ihost(host_id)
    for stor in i_stors:
        if stor.function == constants.STOR_FUNCTION_CINDER:
            ...
            if cinder_disk.device_path:
                ...
                LOG.info("host %s cinder device_path does not exist, return "
                         "device_node=%s" %
                         (host_id, cinder_disk.device_node))

    return cinder_device


def _get_cinder_device_info(dbapi, host_id):
    if not host_id:
        LOG.error("_get_cinder_device: host not defined. ")
        return
    ...
    lvgs = dbapi.ilvg_get_by_ihost(host_id)
    for vg in lvgs:
        if vg.lvm_vg_name == constants.LVG_CINDER_VOLUMES:
            pvs = dbapi.ipv_get_by_ihost(host_id)
            for pv in pvs:
                if pv.forilvgid == vg.id:
                    # NOTE: Only supporting a single PV for cinder volumes
                    if cinder_device:
                        LOG.error("Another cinder device? ignoring! pv: %s" %
                                  pv.uuid)
                        continue
                    cinder_device = pv.disk_or_part_device_path

                    # NOTE: Should only ever be a single partition until we
                    #       support multiple PVs for cinder. Cinder device
                    #       should not be a disk.  Log an error and continue
                    ...
                    try:
                        ...
                        LOG.error("Discovered cinder device is "
                                  "not a partition.")
