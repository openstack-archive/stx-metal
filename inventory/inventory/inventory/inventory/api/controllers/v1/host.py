...
# from inventory.api.controllers.v1 import profile
...
from inventory.api.controllers.v1 import mtce_api
from inventory.api.controllers.v1 import node as node_api
from inventory.api.controllers.v1 import patch_api
...
from inventory.api.controllers.v1 import sm_api
from inventory.api.controllers.v1 import types
from inventory.api.controllers.v1 import utils
from inventory.api.controllers.v1 import vim_api
...
# to be removed StorageBackendConfig
...
class Host(base.APIBase):
    ...
    def convert_with_links(cls, rpc_ihost, expand=True):
        minimum_fields = [
            ...
            'ihost_action', 'mgmt_mac', 'mgmt_ip', 'infra_ip', 'location',
        ...
        # JK obsol noref Don't expose the vsc_controllers field if
        # we are not configured with
...
class HostController(rest.RestController):
    ...
    "Expose nodes as a sub-element of ihosts"
    ...
    def bulk_add(self):
        ...
        controller_list = objects.Host.list(
            pecan.request.context,
            filters={'personality': constants.CONTROLLER})
    ...
    def bulk_export(self):
        def host_personality_name_sort_key(host_obj):
            if host_obj.personality == constants.CONTROLLER:
                rank = 0
            elif host_obj.personality == constants.STORAGE:
                rank = 1
            elif host_obj.personality == constants.COMPUTE:
                rank = 2
            else:
                rank = 3
    ...
    def _patch(self, uuid, patch, myprofile_uuid):
        ...
        if (constants.TASK_REINSTALLING == ihost_obj.task and
                constants.CONFIG_STATUS_REINSTALL == ihost_obj.config_status):
    ...
    def delete(self, host_id):
        ...
        if ihost.administrative == constants.ADMIN_UNLOCKED:
            ...
            raise exception.HostLocked(
                action=constants.DELETE_ACTION, host=host)
        ...
        if mtce_response.get('status') != 'pass':
            ...
            self._handle_mtce_response(constants.DELETE_ACTION,
                                       mtce_response)
    ...
    def _controller_storage_node_setup(self, patched_ihost, hostupdate=None):
        ...
        if patched_ihost['personality'] == constants.CONTROLLER:
            ...
            current_storage_ihosts = objects.Host.list(
                pecan.request.context,
                filters={'personality': constants.STORAGE})
    ...
    def _semantic_check_vsc_controllers(ihost, vsc_controllers):
        ...
        # TODO(sc): checks this on provisioning and new host create
        vswitch_type = utils.get_vswitch_type()
        if (vswitch_type != constants.VSWITCH_TYPE_NUAGE_VRS or
        ...
        if ('administrative' in ihost and
        ...
        if vsc_controllers:
            ...
            for vsc_ip_str in vsc_list:
                try:
                    ...
                    raise wsme.exc.ClientSideError(
                        _("Rejected: invalid VSC controller "
                          "IP address: {}").format(vsc_ip_str))
            ...
            if rc:
                ...
                try:
                    ...
                    LOG.info("host unlock check didn't pass, "
                             "so set the ihost_action back to None "
    ...
    def _check_apply_profile(hostupdate):
        ...
        if (ihost['administrative'] == constants.ADMIN_UNLOCKED and
            ...
            raise wsme.exc.ClientSideError(
                _("Can not apply profile to an 'unlocked' host {}; "
    ...
    def stage_action(self, action, hostupdate):
        ...
        rc = True
        if not action or (
                action and action.lower() == constants.NONE_ACTION):
    ...
    def _stage_apply_profile_action(hostupdate):
        ...
        LOG.info("TODO SKIP %s _stage_apply_profile_action uuid=%s "
                 "profile_uuid=%s" %
                 (hostupdate.displayid,
                  hostupdate.ihost_patch['uuid'],
                  hostupdate.iprofile_uuid))
        # TODO(sc) profile.apply_profile(hostupdate.ihost_patch['uuid'],
        # TODO(sc)                       hostupdate.iprofile_uuid)
