...
class ConductorManager(base_manager.BaseConductorManager):
    ...
    def handle_dhcp_lease_from_clone(self, context, mac):
        ...
        controller_hosts = \
            self.dbapi.host_get_by_personality(constants.CONTROLLER)
    ...
    def create_ihost(self, context, values, reason=None):
        ...
        values.update({constants.HOST_ACTION_STATE:
                       constants.HAS_REINSTALLING})

        # get tboot value from the active controller
        active_controller = None
        hosts = self.dbapi.host_get_by_personality(constants.CONTROLLER)
        for h in hosts:
        ...
        LOG.info("RPC create_ihost called and created ihost %s." % ihost_id)
    ...
    def port_update_by_host(self, context,
        ...
        if ihost.invprovision not in [constants.PROVISIONED,
        ...
        if host.administrative == constants.ADMIN_UNLOCKED:
            ...
            if (ihost_action_str.startswith(constants.FORCE_LOCK_ACTION) or
                ...
                if (('--' in ihost_action_str and
                    ...
                    if ihost_action_str.startswith(
    ...
    def ibm_deprovision_by_ihost(self, context, host_uuid, ibm_msg_dict):
