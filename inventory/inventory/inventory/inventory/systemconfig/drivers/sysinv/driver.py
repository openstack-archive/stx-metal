...
class SysinvSystemConfigDriver(base.SystemConfigDriverBase):
    ...
    def host_get(self, host_id):
        host = self._client.ihost.get(host_id)
        if not host:
            raise ValueError('No match found for host_id "%s".' % host_id)
        return plugin.Host(host)

    # The following can be deleted as they are for systemconfig side
    def get_hosts_from_systemconfig_JKUNG(self):
        hosts = self._client.host_list()
        return hosts
